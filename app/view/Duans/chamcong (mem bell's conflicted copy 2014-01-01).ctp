		...
		<?php if($liscvs!= null): ?>
			...
			<?php foreach ($nhanviens as $key=>$value): ?>
				...
				<td><?php echo $value['tenNhanVien'];?></td>
					...
					<?php foreach ($liscvs[$key] as $i=>$cv): ?>
						<?php if($i==0): ?>
...
<div class="row index">
	<table cellpadding="0" cellspacing="0" border="0" class="table display" id="example">
		...
		<?php foreach ($nhanviens as $key=>$value): ?>
				 ...
				 <tr>
				<td><?php echo $value['tenNhanVien'];?></td>
				<?php if($listchams[$key]!=null): ?>
					<?php foreach ($listchams[$key] as $i=>$cv): ?>
						<?php if($i==0): ?>
							...
							<?php elseif (!strcmp($cv['Kbcongviec']['cham'],'Đạt')) : ?>
								<td class="chamDat"><?php echo $cv['Kbcongviec']['cham']; ?></td>	
								
							<?php elseif (!strcmp($cv['Kbcongviec']['cham'],'Chậm')) : ?>
